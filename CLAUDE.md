...
- `packages/core-api/` — reusable backend modules (routes + Mongoose models); see its `README.md` for the module convention (`{ name, registerRoutes(app, ctx), models }`) that `backend/server.js` mounts, and for which modules live there today (mail, uploads, products, inventory, orders) vs. what's staying in `backend/` for a future iteration (Auth/Users, StoreConfig) or permanently (AgencyClient/Accounting/Invoices/Infra — Duck-Hack's own internal tooling, not a per-store feature).
...
Multer buffers the upload in memory → `sharp` decodes and re-encodes it (strips unexpected metadata, and rejects anything that isn't actually a JPEG/PNG regardless of the client's declared `Content-Type`) → written to disk under `UPLOADS_DIR` (falls back to `/tmp/media-uploads` if that's not writable). `createSingleImageUploadMiddlewares({ fieldName, filePrefix, maxFileSizeMB })` is the factory used by the user profile-image and store-config (logo/team/testimonial) upload routes, still in `backend/`. Product-image uploads (`POST /api/uploads/products-image`) moved to `packages/core-api/modules/uploads.js`, which keeps its own copy of this factory (`packages/core-api/lib/uploads.js`) — deliberately duplicated rather than imported, since the package can't depend on `backend/` files (see its README's "Scope boundary"). Keep both in sync if you touch the image-processing logic, until Auth/StoreConfig also move here and this can collapse into one copy.
