...
Panel administrativo de Duck-Hack Cloud (React + Vite). Migrado de Create React
App a [Vite](https://vite.dev/) porque `react-scripts` ya no recibe mantenimiento
...
Levanta el servidor de desarrollo de Vite en http://localhost:3001 (HMR).
...
- `REACT_APP_STOREFRONT_URL` — link "Ver sitio público" del sidebar.
...
escucha en **8080**). Ver `nginx.conf` para el fallback de SPA y cabeceras.
